...
export class DateTimePickerComponent implements ControlValueAccessor, OnInit, AfterViewInit {
    ...
    onDateChange($event: NgbDateStruct) {
      let event_temp = null;
        if ($event.year) {
          event_temp = `${$event.year}-${$event.month}-${$event.day}`;
        }

        const date = DateTimeModel.fromLocalString(event_temp);
